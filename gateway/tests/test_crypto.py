...
FRAME_CT = "e0f2ae2cf99b63be99295ff2b5cbb509ff6757ad"
FRAME_TAG = "3c1b295dfd4b9779e0e00d45d10090a3"
PLAINTEXT = b'{"n":9,"s":12,"b":1}'
...
def test_envelope_matches_firmware():
    ...
    ciphertext, tag = crypto._xchacha_seal(
        crypto.derive_field_key(), nonce, bytes([9]), PLAINTEXT)
...
def test_public_aead_round_trip():
    ...
    nonce = bytes(range(24))
    ciphertext, tag = crypto.xchacha_seal(key, nonce, b"\x09", PLAINTEXT)
    opened = crypto.xchacha_open(key, nonce, b"\x09", ciphertext, tag)
