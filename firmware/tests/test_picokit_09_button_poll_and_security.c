/**
 * FILE: test_picokit_09_button_poll_and_security.c
 *
 * DESCRIPTION:
 * Native unit tests for the RP2350 Picokit button poll node: provisioning
 * constants, the packet artifact, CRC, the RYLR998 AT interface, the
 * debounced button input, and the button poll state machine that reflects
 * the switch state on the lamps with an authenticated LoRa heartbeat.
 *
 * BRIEF:
 * Native unit test runner for picokit-09-button-poll.
 *
 * AUTHOR: Kevin Thomas
 * DATE: September 2026
 */
...
static void assert_pin_constants(void) {
    TEST_ASSERT_EQUAL_UINT(25u, PICOKIT_09_BUTTON_POLL_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(8u, PICOKIT_09_BUTTON_POLL_UART_TX);
    TEST_ASSERT_EQUAL_UINT(9u, PICOKIT_09_BUTTON_POLL_UART_RX);
    TEST_ASSERT_EQUAL_UINT(16u, PICOKIT_09_BUTTON_POLL_RED_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(17u, PICOKIT_09_BUTTON_POLL_YELLOW_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(18u, PICOKIT_09_BUTTON_POLL_GREEN_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(15u, PICOKIT_09_BUTTON_POLL_BUTTON_PIN);
}
...
static void assert_frame_constants(void) {
    TEST_ASSERT_EQUAL_UINT(115200u, PICOKIT_09_BUTTON_POLL_UART_BAUD);
    TEST_ASSERT_EQUAL_UINT(48u, PICOKIT_09_BUTTON_POLL_FRAME_SIZE);
    TEST_ASSERT_EQUAL_UINT(5000u, PICOKIT_09_BUTTON_POLL_TX_INTERVAL_MS);
...
static void assert_artifact_ids(void) {
    ...
    TEST_ASSERT_EQUAL_UINT(9u, PACKET_NODE_ID);
...
static void assert_artifact_example(void) {
    ...
    TEST_ASSERT_EQUAL_UINT8(0x39u, PACKET_EXAMPLE_FRAME[5]);
    TEST_ASSERT_EQUAL_UINT8(0x31u, PACKET_EXAMPLE_FRAME[17]);
...
static void press_button(void) {
    gpio_put(PICOKIT_09_BUTTON_POLL_BUTTON_PIN, false);
}
...
static void release_button(void) {
    gpio_put(PICOKIT_09_BUTTON_POLL_BUTTON_PIN, true);
}

/**
 * @brief Service one poll tick and assert the sampled button state.
 *
 * @param at Absolute fake-clock time in microseconds.
 * @param expected_button Expected sampled button state after the tick.
 * @return void
 */
static void poll_state_at(uint64_t at, unsigned expected_button) {
    step_at(at);
    TEST_ASSERT_EQUAL_UINT(expected_button, (unsigned)g_button);
}
...
void test_monitor_build_frame_json(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("{\"n\":9,\"s\":12,\"b\":1}", frame);
}
...
void test_monitor_init(void) {
    ...
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[PICOKIT_09_BUTTON_POLL_LED_PIN]);
    TEST_ASSERT_FALSE(s_mock_gpio_dirs[PICOKIT_09_BUTTON_POLL_BUTTON_PIN]);
}

void test_monitor_poll(void) {
    uint64_t base;
    init_monitor();
    base = mock_timer_now_us();
    release_button();
    poll_state_at(base, 0u);
    TEST_ASSERT_EQUAL_UINT(1u, (unsigned)g_step);
}

void test_monitor_pressed(void) {
    uint64_t base;
    init_monitor();
    base = mock_timer_now_us();
    press_button();
    poll_state_at(base, 1u);
    TEST_ASSERT_EQUAL_INT(1, mock_gpio_get(PICOKIT_09_BUTTON_POLL_RED_LED_PIN));
    TEST_ASSERT_EQUAL_INT(1, mock_gpio_get(PICOKIT_09_BUTTON_POLL_GREEN_LED_PIN));
}
...
/**
 * @brief Run the button poll monitor state-machine tests.
 *
 * @param void No parameters.
 * @return void
 */
static void run_monitor_tests(void) {
    ...
    RUN_TEST(test_monitor_poll);
    RUN_TEST(test_monitor_pressed);
